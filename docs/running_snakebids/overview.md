...
1. Uncomment the lines in `snakebids.yml` containing `pybidsdb_dir` and `pybidsdb_reset`.
1. The variables can be updated directly in this file or through the CLI by using `-pybidsdb-dir {dir}` to specify the database path and `--pybidsdb_reset` to indicate that the database should be updated. _Note: CLI arguments take precendence if both CLI and config variables are set._
