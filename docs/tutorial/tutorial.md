...
The first thing we can make use of is the [`bids()`](snakebids.bids) function. This provides an easy way to generate bids filenames. This is especially useful when defining output files in your workflow and you have many bids entities.

In our existing workflow, this was our output file::

    'results/sub-{subject}/func/sub-{subject}_task-{task}_run-{run}_fwhm-{fwhm}mm_bold.nii.gz'

To create the same path using [`bids()`](snakebids.bids), we just need to specify the root directory (``results``), all the bids tags (subject, task, run, fwhm), and the suffix (which includes the extension)::
...
```{note}
The entities you supply in the [`bids()`](snakebids.bids) function do not have to be in the BIDS specification, e.g. ``fwhm`` is not in the spec. But if you do use entities that are in the BIDS specification, snakebids will ensure they go in the correct order (e.g. `sub_*-ses_*-run_*...`). Non-standard entities will be added to the end of the path, just before the suffix, in the order they were defined.
```
...
To add this parsing to the workflow, we call the {func}`snakebids.generate_inputs()` function before our rules are defined, and pass along some configuration data to specify the location of the bids directory (``bids_dir``), and the inputs we want to parse for the workflow (``pybids_inputs``). The function returns a {class}`BidsInputs() <snakebids.BidsInputs>`, which we'll assign to a variable called ``inputs``::
...
The [`BidsInputs`](snakebids.BidsInputs) class returned by [`generate_inputs()`](snakebids.generate_inputs) summarizes the wildcards found in your bids dataset and has parameters to plug those wildcards into your workflow. The first parameter we'll use is [`BidsInputs.input_path`](snakebids.BidsInputs.input_path). You can see what it looks like by printing it after calling [`generate_inputs()`](snakebids.generate_inputs):
...
%TODO: add asciinema
If you ran the workflow, you'd see that `input_path` is a `dict` with one entry for each named input we specifed under `pybids_config`. In this case, since we only specified one such input: `bold`, `input_path` has just one item. The values of this dict are generic paths constructed using the wildcards we specified in `pybids_config`. In this case, the path has a `{subject}`, `{session}`, `{acq}`, `{task}`, and `{run}` wildcard. Notice that this path is the same as the path we wrote under `in_bold:` in our `config.yaml` file in [step 4](step_4). In fact, we can go ahead and replace `config['in_bold']` in our `Snakemake` file with `inputs.input_path['bold]` and delete `in_bold` from `config.yaml`.
...
[`BidsInputs.input_path`](snakebids.BidsInputs.input_path) already grants us a lot of flexibility, but we can still do more! Next we'll look at [`BidsInputs.input_wildcards`](snakebids.BidsInputs.input_wildcards). This is a dict that maps our input names to the brace-wrapped `{wildcards}` we specified in `pybids_config`. If you printed this value in our test workflow, it would look like this:

```py
inputs.input_wildcards == {
    'bold': {
        'subject': '{subject}',
        'task': '{task}',
        'run': '{run}'
...
This is super useful when combined with ``bids()``, as we can use the keyword expansion (``**inputs.input_wildcards[<input_name>]`` to set all the wildcard parameters to the ``bids()`` function. Thus, we can make our workflow even more general, by replacing:
...
Another useful config variable is [`input_lists`](snakebids.BidsInputs.input_lists). This is indexed by the pybids input type (e.g. `bold`) and has the list of values that each wildcard can take:
...
```{note}
Since ``expand()`` will by default use all combinations of these, this can lead to snakemake searching for inputs that do not exist (e.g. if only one of your subjects has run-2). In this case, you want to use the [`BidsInputs.input_zip_lists`](snakebids.BidsInputs.input_zip_lists) instead).
...
We do this in snakebids by creating an executable python script, which uses the [`SnakeBidsApp`](snakebids.app.SnakeBidsApp) class from `snakebids.app` to run snakemake. An example of this ``run.py`` script is shown below.
